...
export function StatsDashboard() {
  ...
  const { data: dealsData } = useQuery({
    queryKey: ["deals", "all"],
    queryFn: () => authFetch(`${API}/api/deals?limit=500`).then((r) => r.json()),
    staleTime: 60_000,
  })
  ...
  return (
    <div>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
          ...
          {expectedCount === 0 ? (
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto">
              {deals.filter((d: any) => (d.amount || 0) - (d.paid || 0) > 0).slice(0, 15).map((d: any) => {
                const remainder = (d.amount || 0) - (d.paid || 0)
                return (
                  <div key={d.id} className="flex items-center justify-between p-2.5 rounded-lg bg-zinc-900 border border-zinc-800">
              ...
              {deals.filter((d: any) => (d.amount || 0) - (d.paid || 0) > 0).length > 15 && (
                <p className="text-xs text-zinc-600 text-center">... и ещё</p>
              )}
            </div>
          )}
        </div>
      ...
      {/* Placeholder widgets for future metrics */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <PlaceholderCard title="Шкала Шумкина" subtitle="Результаты женского теста" />
        <PlaceholderCard title="Мужской тест" subtitle="Результаты мужского теста" />
        <PlaceholderCard title="Конверсия воронки" subtitle="Среднее время по этапам" />
      </div>
    </div>
  )
}
...
function PlaceholderCard({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="rounded-xl border border-dashed border-zinc-800 bg-zinc-900/20 p-5 flex flex-col items-center justify-center h-28 gap-2">
      ...
      <span className="text-[9px] text-zinc-700 italic">скоро</span>
    </div>
  )
}
