...
interface AssistantTaskSheetProps {
  taskId: number | null
  onClose: () => void
}

export function AssistantTaskSheet({ taskId, onClose }: AssistantTaskSheetProps) {
  ...
  const { data: tasksData } = useQuery({
    ...
    queryFn: () => authFetch(`${API}/api/assistant-tasks`).then((r) => r.json()),
  })

  const task: AssistantTask | undefined = tasksData?.tasks?.find((t: AssistantTask) => t.id === taskId)

  const { data: commentsData } = useQuery({
    queryKey: ["assistant-task-comments", taskId],
    queryFn: () => authFetch(`${API}/api/assistant-tasks/${taskId}/comments`).then((r) => r.json()),
    enabled: !!taskId,
  })
  ...
  const addComment = useMutation({
    mutationFn: (text: string) =>
      authFetch(`${API}/api/assistant-tasks/${taskId}/comments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text, author: "" }),
      }).then((r) => r.json()),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["assistant-task-comments", taskId] })
      queryClient.invalidateQueries({ queryKey: ["assistant-tasks"] })
    },
  })
  ...
  const deleteTask = useMutation({
    mutationFn: () =>
      authFetch(`${API}/api/assistant-tasks/${taskId}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["assistant-tasks"] })
      onClose()
    },
  ...
  const deleteComment = useMutation({
    mutationFn: (commentId: number) =>
      authFetch(`${API}/api/assistant-tasks/${taskId}/comments/${commentId}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["assistant-task-comments", taskId] })
      queryClient.invalidateQueries({ queryKey: ["assistant-tasks"] })
    },
        ...
        {task ? (
          <>
            ...
            <ScrollArea className="flex-1 px-8 py-5">
              ...
              {/* Checklist */}
              <TaskChecklist taskType="assistant" taskId={taskId} />

              {/* Files */}
              <TaskFiles taskType="assistant" taskId={taskId} />
