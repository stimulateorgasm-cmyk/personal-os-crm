...
const TASK_TYPES = [
  ...
  { value: "session", label: "Сессия", icon: MessageCircle, color: "text-violet-400" },
      ...
      {showForm && (
        <div className="space-y-2 mb-3 p-3 rounded-lg bg-zinc-900 border border-zinc-800">
          <Input
            ...
            className="h-10 text-sm bg-zinc-950 border-zinc-800 w-full"
          />
          <div className="flex flex-col sm:flex-row gap-2">
            <Select value={newType} onValueChange={setNewType}>
              <SelectTrigger className="h-10 text-xs bg-zinc-950 border-zinc-800 w-full sm:w-[130px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {TASK_TYPES.map((t) => (
            ...
            <Input
              ...
              className="h-10 text-sm bg-zinc-950 border-zinc-800 w-full sm:w-[130px]"
            />
            <Button
              size="default"
            ...
            >
              {addTask.isPending ? <Loader2 size={14} className="animate-spin" /> : <><StickyNote size={14} className="mr-1" /> Создать</>}
            </Button>
          </div>
        </div>
      )}
        ...
        return (
          <div key={t.id} className="flex items-start gap-2.5 py-1.5 group">
            <button
              ...
              className="shrink-0 text-zinc-600 hover:text-emerald-400 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center touch-manipulation"
            >
              <Circle size={16} />
            </button>
