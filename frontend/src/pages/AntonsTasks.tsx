...
const STAGE_COLORS: Record<string, { headerBg: string; headerText: string; dotColor: string; accentBorder: string }> = {
  "Идеи":               { headerBg: "bg-zinc-900/80",     headerText: "text-zinc-300",  dotColor: "bg-zinc-500",   accentBorder: "border-t-zinc-500/40" },
...
function StageColumn({ stage, children, count, total, onAdd }: { stage: string; children: React.ReactNode; count: number; total: number; onAdd: () => void }) {
  const colors = STAGE_COLORS[stage] || STAGE_COLORS["Идеи"]
...
function InlineCreateInput({ stage, onDone }: { stage: string; onDone: () => void }) {
  ...
  const createTask = useMutation({
    mutationFn: (title: string) =>
      authFetch(`${API}/api/antons-tasks`, {
        method: "POST",
...
// ─── Main component ────────────────────────────────────────────────────────────
export function AntonsTasks({ onSelect }: AntonsTasksProps) {
  const queryClient = useQueryClient()
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["antons-tasks", showArchived],
    queryFn: () => authFetch(`${API}/api/antons-tasks${showArchived ? '?archived=true' : ''}`).then((r) => r.json()),
    staleTime: 30_000,
  })

  const updateTask = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) =>
      authFetch(`${API}/api/antons-tasks/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const reorderTasks = useMutation({
    mutationFn: ({ status, orderedIds }: { status: string; orderedIds: number[] }) =>
      authFetch(`${API}/api/antons-tasks/reorder`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          status,
  ...
  const byStatus: Record<string, AntonTask[]> = useMemo(() => {
    const map: Record<string, AntonTask[]> = {}
  ...
  if (isLoading) {
    return (
      <div>
        <div className="mb-6">
          <h1 className="text-xl font-semibold text-white">Задачи Антона</h1>
...
// ─── Archived View ────────────────────────────────────────────────────────────
function ArchivedView({ tasks, onSelect }: { tasks: AntonTask[]; onSelect: (id: number) => void }) {
